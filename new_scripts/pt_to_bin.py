...
parser.add_argument("--directory", type=str, help="Which model")
...
# model_state_dict = torch.load(f'.cache/laura/{args.directory}/policy.pt') # .to(torch.device('cuda'))
# torch.save(dict(model_state_dict["state"]), f".cache/laura/{args.directory}/pytorch_model.bin")
model_state_dict = torch.load(f'{args.directory}/policy.pt') # .to(torch.device('cuda'))
torch.save(dict(model_state_dict["state"]), f"{args.directory}/pytorch_model.bin")
